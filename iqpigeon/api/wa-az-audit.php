...
$sendBeforeAck = az_has($webhook, 'Inline send before Meta ACK')
...
$report = [
    ...
    'time' => date('c'),
    'php'  => PHP_VERSION,
    'hint' => 'Default is core only (no 503). Add &part=qualify | orders | logs | compose. Add &fix=1 to send waiting chats.',
];
...
if ($part === 'core' || $part === 'all') {
    $report['a_files'] = [
        ...
        'send_before_meta_ack' => $sendBeforeAck,
        ...
        'quiet_wait_5s'        => az_has($engine, '$maxWaitMs = 8000'),
        'wait_quiet_no_typing' => az_has($engine, 'Silent wait until 5s after last bubble'),
    ...
    $blockers = [];
    if (!$sendBeforeAck) {
        $blockers[] = 'Webhook on this server still does not send before Meta ACK. Re-upload api/whatsapp-webhook.php and includes/conversation-turn-engine.php.';
    }
    ...
    $report['verdict'] = $blockers === []
        ? ['status' => 'healthy', 'summary' => 'Send-before-ACK is on disk. New inbound should reply in the webhook request.']
