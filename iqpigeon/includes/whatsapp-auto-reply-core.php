...
function wa_webhook_products(int $botId, int $offset = 0, int $limit = 12): array
{
    ...
    try {
        ...
        error_log('wa_webhook_products: ' . $e->getMessage());
...
function wa_webhook_wants_catalog(string $msg, array $bot = []): bool
{
    ...
    if (preg_match('/\b(menu|catalog|catalogue|price list|checkout|add to cart|place order)\b/u', $msg)) {
        return true;
    }
    if (preg_match('/\b(show|send|see|browse)\b.{0,40}\b(menu|items|products|catalog|catalogue|services|packages)\b/u', $msg)) {
        return true;
    }
...
function wa_webhook_menu_text(array $bot, array $products): string
{
    ...
    $lines = ['Here is the ' . $brand . ' list — reply with a number or the name to add it. Tap *View catalog* for photos of the full catalog.'];
    foreach ($products as $i => $p) {
        $n = $i + 1;
        $price = (float) ($p['price'] ?? 0);
        $cur = strtoupper(trim((string) ($p['currency'] ?? 'PKR'))) ?: 'PKR';
        $priceTxt = $price > 0 ? ($cur === 'PKR' ? 'PKR ' . number_format($price, 0) : $cur . ' ' . number_format($price, 2)) : '';
        $lines[] = $n . '. ' . trim((string) ($p['name'] ?? 'Item')) . ($priceTxt !== '' ? ' — ' . $priceTxt : '');
    }
