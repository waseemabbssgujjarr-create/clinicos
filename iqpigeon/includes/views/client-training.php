...
$tabs = [
    ['business',   'Business Info',          '<rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/>'],
    ...
    ['industry',   'Industry',               '<path d="M2 20h20M4 20V10l8-7 8 7v10M10 20v-5h4v5"/>'],
    ['qualify',    'Qualify',                '<path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/>'],
    ['menu',       'Menu / Catalogue Items', '<path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1"/><path d="M9 12h6M9 16h4"/>'],
    ['triggers',   'Trigger Words & Files',  '<path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>'],
    ['hours',      'Operating Hours',        '<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>'],
    ['behavior',   'Behavior',               '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>'],
];

$industries = [
    'Restaurant / Food & Beverage','Retail / E-Commerce','Healthcare / Medical',
    'Education / Training','Real Estate','Finance / Banking',
    'Technology / Software','Beauty / Salon','Other',
];
...
if (isset($_GET['applied']) && (string) $_GET['applied'] === '1') {
    if (isset($_GET['kept_qualify']) && (string) $_GET['kept_qualify'] === '1') {
        iqp_flash('Industry template applied. Your custom Qualify flow was kept — reset it from Training → Qualify if you want the template questions.');
    } else {
        iqp_flash('Industry template applied. Order stages, replies, and starter qualifying questions now follow this industry.');
    }
}
if ($tab === 'qualify' && isset($_GET['saved']) && (string) $_GET['saved'] === '1') {
    iqp_flash('Qualification flow saved. Live chats now use your questions and conversion goal.');
}
if ($tab === 'qualify' && isset($_GET['reset']) && (string) $_GET['reset'] === '1') {
    iqp_flash('Qualification reset to the industry starter. Save again after any edits to keep it as your own flow.');
}
?>
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
  <div>
    <h1 class="text-[20px] lg:text-[22px] font-bold text-slate-800">Train Your Sales Representative</h1>
    <p class="text-[13px] text-slate-500 mt-0.5">Teach them your business so they can talk to customers like a live teammate — across every industry.</p>
  </div>
  <div class="flex items-center gap-3 flex-wrap justify-end">
    ...
    <?php endif; ?>
    <button type="button" id="iqpTrainBtn" onclick="trainAssistant()"
      class="flex items-center gap-2 bg-[#1FA855] text-white rounded-lg px-4 py-2.5 text-[13px] font-semibold hover:bg-[#18934a]">
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
      Train My Assistant
    </button>
  </div>
</div>
  ...
  <form id="bizInfoForm" method="POST" enctype="multipart/form-data">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <div class="relative">
          <select name="industry" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-[13px] text-slate-700 bg-white focus:outline-none focus:border-[#1FA855] appearance-none">
            <?php foreach ($industries as $ind): ?>
            <option<?= $industry === $ind ? ' selected' : '' ?>><?= sanitize($ind) ?></option>
            <?php endforeach; ?>
            <?php if ($industry !== '' && !in_array($industry, $industries, true)): ?>
            <option selected><?= sanitize($industry) ?></option>
            <?php endif; ?>
          </select>
        ...
        <p class="text-[10.5px] text-slate-400 mt-1.5">For AI templates and sample replies, go to the <a href="/client/training?tab=industry" class="text-[#1FA855] font-medium hover:underline">Industry tab</a> and apply a template.</p>
      </div>
      <div>
        <label class="block text-[11.5px] text-slate-500 font-medium mb-1.5">Business Email</label>
        <input name="business_email" value="<?= sanitize((string)($user['email']??'')) ?>" placeholder="info@yourcompany.com"
          class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-[13px] text-slate-700 focus:outline-none focus:border-[#1FA855]"/>
      </div>
      <div>
      ...
      <div>
        ...
        <div class="border-2 border-dashed border-slate-200 rounded-lg p-4 text-center cursor-pointer hover:border-[#1FA855] hover:bg-emerald-50/20 transition-colors"
          onclick="document.getElementById('bizLogoInput').click()">
          <?php $curLogo = (string) ($user['avatar_url'] ?? ''); ?>
          <?php if ($curLogo !== ''): ?>
          <img id="bizLogoPreview" src="<?= sanitize($curLogo) ?>" class="w-12 h-12 rounded-lg object-cover mx-auto mb-1.5" alt="Logo"/>
          <?php else: ?>
          <svg id="bizLogoIcon" class="mx-auto mb-1.5 text-slate-300" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
          <?php endif; ?>
          <div id="bizLogoFilename" class="text-[11.5px] text-slate-400">JPG, PNG, SVG · Max size 2MB.</div>
        ...
        <button type="button" onclick="document.getElementById('bizLogoInput').click()"
          class="mt-2 w-full border border-slate-200 rounded-lg py-1.5 text-[12px] font-medium text-slate-600 hover:bg-slate-50">
          Change Logo
        </button>
      ...
      <div>
        ...
        <div class="relative">
          <select name="timezone" class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-[13px] text-slate-700 bg-white focus:outline-none focus:border-[#1FA855] appearance-none">
            <option selected>(GMT+05:00) Islamabad, Karachi</option>
...
<div class="bg-white rounded-xl border border-slate-200 p-5 max-w-5xl">
  <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
    <div>
      <div class="text-[15px] font-bold text-slate-800">Knowledge Base</div>
      <div class="text-[12px] text-slate-400 mt-0.5">Policies, FAQs, hours, and business details your AI uses on every reply.</div>
    </div>
    <?php if ($updated !== ''): ?>
  ...
  <form method="POST">
    ...
    <p class="text-[11px] text-slate-400 mt-3">Industry templates and Shop products add extra context automatically — edit your core story here. Qualifying questions live in <a href="/client/training?tab=qualify" class="text-[#1FA855] font-medium">Qualify</a>, not in this document.</p>
  </form>
</div>
...
<div class="bg-white rounded-xl border border-slate-200 p-5">
  <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
    <div>
      <div class="text-[15px] font-bold text-slate-800">Menu Cards</div>
      <div class="text-[12px] text-slate-400 mt-0.5">Pick items from your Shop catalog — grouped into WhatsApp menu cards by category (multiple cards per category allowed).</div>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 p-5 max-w-5xl">
  <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
    <div>
      <div class="text-[15px] font-bold text-slate-800">Qualification flow</div>
      <div class="text-[12px] text-slate-400 mt-0.5">This is yours only. Admin industry templates stay global starters — save here to change what this bot asks and when a lead counts as qualified.</div>
      <?php if ($qLabel === ''): ?>
      <div class="text-[12px] text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 mt-2">No industry template applied yet. You can still build a flow here, or pick a starter on the <a href="/client/training?tab=industry" class="font-semibold underline">Industry</a> tab first.</div>
      <?php endif; ?>
    </div>
  ...
  <form method="POST" id="qualifyForm">
    ...
    <div class="mb-5">
      <div class="flex items-center justify-between gap-2 mb-2">
        <div>
          <div class="text-[13px] font-semibold text-slate-800">Qualifying questions</div>
          <div class="text-[11.5px] text-slate-400">Asked one at a time after small talk. Required questions must be answered before the bot qualifies them.</div>
        </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <button type="submit" class="bg-[#1FA855] text-white rounded-lg px-5 py-2.5 text-[13px] font-semibold hover:bg-[#18934a]">Save qualification</button>
    </div>
  </form>
...
<div class="bg-white rounded-xl border border-slate-200 p-5">
  <div class="flex items-center justify-between mb-4">
    <div>
      <div class="text-[15px] font-bold text-slate-800">Trigger Words</div>
      <div class="text-[12px] text-slate-400 mt-0.5">Keywords that hint the AI toward a specific intent (e.g. "menu" → Show Menu). The AI still uses judgment — these are hints, not hard rules.</div>
    </div>
  </div>
  ...
  <form method="POST" class="flex flex-wrap items-end gap-2 mb-5 bg-slate-50 border border-slate-100 rounded-lg p-3">
    ...
    <div class="flex-1 min-w-[140px]">
      <label class="block text-[11px] text-slate-500 font-medium mb-1">Trigger Word / Phrase</label>
...
<!-- ---- Behavior Tab ---- -->
<div class="bg-white rounded-xl border border-slate-200 p-5">
  <div class="text-[15px] font-bold text-slate-800 mb-0.5">AI Assistant Behavior</div>
  <div class="text-[12px] text-slate-400 mb-4">Configure how your AI assistant responds when the store is closed.</div>
  <form method="POST">
    <input type="hidden" name="csrf_token" value="<?= sanitize($csrf) ?>"/>
    <input type="hidden" name="action" value="save_closed_behavior"/>
    <div class="mb-4">
      <div class="text-[13px] font-semibold text-slate-800 mb-1">AI Assistant Behavior (When Store is Closed)</div>
      <div class="text-[12px] text-slate-400 mb-3">The assistant always replies 24/7 but lets customers know you're closed, using the note below. Only applies if "Always Open" is off in Operating Hours.</div>
      ...
      <textarea id="closedBehaviorInput" name="closed_behavior" maxlength="500" rows="4"
        class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-[13px] text-slate-700 focus:outline-none focus:border-[#1FA855] resize-none"
        placeholder="Sorry, we are currently closed. Our operating hours are {operating_hours}. Please visit us during business hours or contact {store_name} for assistance."><?= sanitize((string) ($trainingMeta['closed_behavior'] ?? '')) ?></textarea>
    </div>
    <div class="flex justify-end">
      <button class="bg-[#1FA855] text-white rounded-lg px-5 py-2.5 text-[13px] font-semibold hover:bg-[#18934a]">Save Changes</button>
    </div>
  </form>
</div>
<?php endif; ?>
...
<!-- ==================== RIGHT RAIL: Test Your Assistant ==================== -->
<div class="iqp-train-rail space-y-4 min-w-0">

  <!-- Name Your Assistant (Business Info tab only) -->
  <?php if ($tab === 'business'): ?>
  <div class="bg-white rounded-xl border border-slate-200 p-4 min-w-0">
    <div class="text-[12.5px] font-semibold text-slate-700 mb-2">Name Your Assistant</div>
    <form method="POST" class="iqp-assistant-name-form">
      <input type="hidden" name="csrf_token" value="<?= sanitize($csrf) ?>"/>
      <input type="hidden" name="action" value="save_business_info"/>
      <input name="rep_name" value="<?= sanitize($repName) ?>" maxlength="30" placeholder="e.g. Sara, Alex"
        class="w-full border border-slate-200 rounded-lg px-3 py-2 text-[13px] text-slate-700 focus:outline-none focus:border-[#1FA855] min-h-[40px]"/>
      <button type="submit" class="iqp-train-save-btn bg-[#1FA855] text-white rounded-lg px-4 py-2 text-[12.5px] font-semibold hover:bg-[#18934a]">Save</button>
    </form>
  </div>
  <?php endif; ?>

  <!-- Test Assistant chat panel -->
  <div class="bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col" style="height:600px">
    <div class="px-4 py-3.5 border-b border-slate-100">
      <div class="text-[14px] font-bold text-slate-800">Test Your Assistant</div>
      <div class="text-[11.5px] text-slate-400 mt-0.5">Chat with your AI Assistant to see how it responds.</div>
    </div>

    <!-- Chat header -->
    <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
      <div class="flex items-center gap-2.5">
        <div class="w-9 h-9 rounded-full bg-[#1FA855] flex items-center justify-center shrink-0">
          <span class="text-white text-[12px] font-bold"><?= sanitize(iqp_initials($repNamePreview)) ?></span>
        </div>
        <div>
      ...
      <button type="button" onclick="resetChat()" class="text-slate-400 hover:text-slate-600">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 4v6h-6"/><path d="M1 20v-6h6"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10"/><path d="M20.49 15a9 9 0 0 1-14.85 3.36L1 14"/></svg>
      </button>
    </div>
    ...
    <div id="chatMessages" class="flex-1 overflow-y-auto px-4 py-3 space-y-3 bg-slate-50/40">
      <!-- Welcome message -->
      <div class="flex gap-2">
        <div class="w-7 h-7 rounded-full bg-[#1FA855] flex items-center justify-center shrink-0 mt-1">
    ...
    <div id="chatChips" class="flex flex-wrap gap-1.5 px-4 py-2 border-t border-slate-100 bg-white">
      <?php foreach (['What are your opening hours?','Do you offer home delivery?','What\'s your best selling item?'] as $chip): ?>
      <button type="button" onclick="sendChatChip(this.textContent)"
        class="border border-slate-200 rounded-full px-2.5 py-1 text-[11px] text-slate-600 hover:bg-slate-50 hover:border-[#1FA855] hover:text-[#1FA855] transition-colors">
        <?= sanitize($chip) ?>
      </button>
      <?php endforeach; ?>
    </div>
    ...
    <div class="flex items-center gap-2 px-4 py-3 border-t border-slate-100 bg-white">
      ...
      <button type="button" onclick="emojiPicker()" class="text-slate-400 hover:text-slate-600">
    ...
    <div class="px-4 py-1.5 text-center text-[10.5px] text-slate-400 border-t border-slate-50 bg-white">
      Responses are AI generated based on your training data
    </div>
  </div>
...
function appendTyping(){
  const wrap = document.getElementById('chatMessages');
  wrap.innerHTML += `<div id="typingIndicator" class="flex gap-2">
    <div class="w-7 h-7 rounded-full bg-[#1FA855] flex items-center justify-center shrink-0">
      <span class="text-white text-[10px] font-bold">${escHtml(repInit)}</span>
    </div>
    <div class="bg-white border border-slate-200 rounded-xl rounded-tl-none px-4 py-3 shadow-sm flex gap-1 items-center">
      <div class="w-2 h-2 rounded-full bg-slate-300 animate-bounce"></div>
      <div class="w-2 h-2 rounded-full bg-slate-300 animate-bounce" style="animation-delay:.15s"></div>
      <div class="w-2 h-2 rounded-full bg-slate-300 animate-bounce" style="animation-delay:.3s"></div>
    </div></div>`;
  wrap.scrollTop = wrap.scrollHeight;
}
...
async function sendChat(){
  ...
  try {
    ...
    removeTyping();
    appendMsg(data.reply || data.message || 'Sorry, I couldn\'t process that.', 'bot');
  } catch(e){
    removeTyping();
...
function resetChat(){
  document.getElementById('chatMessages').innerHTML = `<div class="flex gap-2">
    <div class="w-7 h-7 rounded-full bg-[#1FA855] flex items-center justify-center shrink-0 mt-1">
      <span class="text-white text-[10px] font-bold">${escHtml(repInit)}</span>
    </div>
    <div class="bg-white border border-slate-200 rounded-xl rounded-tl-none px-3 py-2 text-[12.5px] text-slate-700 max-w-[85%] shadow-sm">
...
function iqpPreviewLogo(input){
  ...
  reader.onload = function (e) {
    ...
    if (!img) {
      ...
      img.className = 'w-12 h-12 rounded-lg object-cover mx-auto mb-1.5';
...
// "Train My Assistant" — there is no separate training job. Business info and
// knowledge saved on this page are used as live context on every reply, the
// same way ChatGPT uses custom instructions. This just confirms it's ready.
function trainAssistant(){
  const btn = document.getElementById('iqpTrainBtn');
  if (!btn) return;
  const orig = btn.innerHTML;
  btn.disabled = true;
  btn.innerHTML = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> Ready — no training needed';
