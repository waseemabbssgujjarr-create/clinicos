...
exports.testAI = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const sid = await (0, twilio_service_1.sendWhatsApp)(phone, testMessage, req.clinicId);
    if (!sid) {
...
exports.testAIChat = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { message = 'Hi, I want to book a dental appointment tomorrow' } = req.body;
    const clinic = await prisma_1.prisma.clinic.findUnique({
        where: { id: req.clinicId },
        select: {
    ...
    const result = await (0, ai_service_1.processInboundMessage)({
        ...
        aiPersonality: clinic.aiPersonality || 'professional',
        customIntroMsg: clinic.customIntroMsg || null,
        patientHistory: 'New patient — no prior visits',
        conversationHistory: '',
    }, message);
    res.json({
